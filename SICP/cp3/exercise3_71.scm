...
(define (cube x) (* x x x))
(define (triple_sum item)
  (+ (cube (car item)) (cube (cadr item))))
(define pairs-stream (pairs integers integers triple_sum))

(define (ramanujan stream count)
  (define (iter stream n)
    (if (> n 0)
      (let ((first (stream-car stream))
            (second (stream-car (stream-cdr stream))))
        (if (= (triple_sum first) (triple_sum second))
            (begin
              (display (triple_sum first))
              (display ": ")
              (display first)
              (display second)
              (newline)
